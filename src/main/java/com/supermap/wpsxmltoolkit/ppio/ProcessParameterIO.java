...
//        defaults.add(new CQLFilterPPIO());
    ...
    public static ProcessParameterIO find(Parameter<?> p, String mime) {
        ...
        List<ProcessParameterIO> all = findAll(p);
        if (all.isEmpty()) {
            return null;
        }
    ...
    public static List<ProcessParameterIO> findAll(Parameter<?> p) {
        // enum special treatment
        if (p.type.isEnum()) {
        ...
        // TODO 怎么加载插件？ l.addAll(GeoServerExtensions.extensions(ProcessParameterIO.class));


        // load by factory
        List<PPIOFactory> ppioFactories = new ArrayList<PPIOFactory>();
        // TODO 怎么加载插件？ ppioFactories = GeoServerExtensions.extensions(PPIOFactory.class);
     ...
     */
    private static List<ProcessParameterIO> findByDirection(Parameter<?> p, PPIODirection direction) {
        List<ProcessParameterIO> ppios = new ArrayList<ProcessParameterIO>();
        List<ProcessParameterIO> matches = findAll(p);
     ...
     */
    public static List<ProcessParameterIO> findEncoder(Parameter<?> p) {
        return findByDirection(p, PPIODirection.ENCODING);
    }

    /*
     * Look for PPIO matching the parameter type and suitable for input handling
     */
    public static List<ProcessParameterIO> findDecoder(Parameter<?> p) {
        return findByDirection(p, PPIODirection.DECODING);
    }
     ...
     *
     */
    public static boolean isComplex(Parameter<?> param) {
        List<ProcessParameterIO> ppios = findAll(param);
        if (ppios.isEmpty()) {
            return false;
        } else {
